...
* __formula__ a formula with a minimal structure of `ReCen(time, censor_time)~rand(arm, rx)` where 
...
rpsftm(formula=ReCen(progyrs, censyrs) ~ rand(imm, rx), 
       data=immdef,
       low_psi=1, 
       hi_psi=2)
```
...
```{r, echo=FALSE}
rpsftm_fit <- rpsftm(formula=ReCen(progyrs, censyrs) ~ rand(imm, rx),
                     data=immdef,
                     low_psi=-1, 
                     hi_psi=0)
```
...
```{r}
rpsftm_fit_lr <- rpsftm(formula=ReCen(progyrs, censyrs) ~ rand(imm, rx), 
                        data=immdef)
```
...
```{r}
rpsftm_fit_cph <- rpsftm(formula=ReCen(progyrs, censyrs) ~ rand(imm, rx) + entry, 
                         data=immdef,
                         test=coxph)
...
```{r}
rpsftm_fit_wb <- rpsftm(formula=ReCen(progyrs, censyrs) ~ rand(imm, rx) + entry, 
                        data=immdef,
                        test=survreg)
