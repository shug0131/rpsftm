...
* __censyrs__ censoring time, corresponding to the close of study minus the time of randomisation for each participant
* __xo__ an indicator that crossover occurred
* __xoyrs__ the time from randomisation at which crossover happened, or 0 for participants in the Immediate arm
* __prog__ an indicator of disease progression (1), or censoring (0)
* __progyrs__ time from randomisation of disease progression or censoring
* __entry__ the time from the global start of study of the participant's entry into the study
...
* __rand__ the `rand()` object used to specify the allocated and observed amount of treatment
* __ans__ the values from `uniroot.all` used to solve the estimating equation, but embeded within a list as per `uniroot`, with an extra element `root_all`, a vector of all roots found in the case of multiple solutions. The first element of `root_all` is subsequently used.
...
* Further elements corresponding to either a `survdiff`, `coxph`, or `survreg` object. This will always include:
    ...
    * __terms___ a more detailed representation of the model formula
