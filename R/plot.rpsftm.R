...
plot.rpsftm <- function(x) {
  ...
  ggplot2::ggplot(data = df, ggplot2::aes(x = Time, y = Survival, group = Group, 
                                          lty = Group)) + ggplot2::geom_step() + ggplot2::ylim(0, 1) + ggplot2::labs(title = "KM Plots of Transformed Treatment-Free Time")
