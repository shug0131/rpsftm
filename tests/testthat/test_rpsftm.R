...
test_that("print method",{
  ...
  fit_coxph <- rpsftm(Surv(progyrs, prog)~rand(imm,propX),immdef, censor_time = censyrs, 
                test=coxph)
  fit_survreg <- rpsftm(Surv(progyrs, prog)~rand(imm,propX),immdef, censor_time = censyrs,
                        test=survreg)
...
test_that("detailed print.coxph test",{
  ...
  fit_coxph <- rpsftm(Surv(progyrs, prog)~rand(imm,propX)+cluster(site)+entry,immdef, censor_time = censyrs, 
                      test=coxph)
...
test_that("detailed print.summary.coxph test",{
  site <- rep(1:10,each=100)
  fit_coxph <- rpsftm(Surv(progyrs, prog)~rand(imm,propX)+entry,immdef, censor_time = censyrs, 
                      test=coxph)
...
test_that("detailed print.survreg test",{
  ...
  site <- rep(1:10,each=100)
  fit <- rpsftm(Surv(progyrs, prog)~rand(imm,propX)+entry,immdef, censor_time = censyrs, 
                      test=survreg, scale=1)
  expect_output(print(fit),"Scale fixed at")
 
  
  
  fit <- rpsftm(Surv(progyrs, prog)~rand(imm,propX)+strata(site),immdef, censor_time = censyrs, 
                test=survreg)
...
test_that("first basic fit with the arm as a factor",{
  ...
  fit <- rpsftm(Surv(progyrs, prog)~rand(myArm,propX),immdef, censor_time = censyrs,#formula=~1,
                low_psi=-1, hi_psi=1)
  expect_is(fit$psi, class="numeric")
})
...
test_that("Try it with Censoring off",
          {
            fit  <- rpsftm(Surv(progyrs, prog)~rand(imm,propX),immdef,
                                 low_psi=-1, hi_psi=1)
...
test_that("updating of data argument",
          {
            fit <- rpsftm(Surv(progyrs, prog)~rand(imm,propX),subset(immdef, entry<1))
...
test_that("eval_z output",
          {
            fit <- rpsftm(Surv(progyrs, prog)~rand(imm,propX),
...
{immdef$sqrent <-  with(immdef, sqrt(entry))
 fit1 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+sqrent, 
       data=immdef, censor_time=censyrs, test=coxph)
 fit2 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+sqrt(entry), 
              data=immdef, censor_time=censyrs, test=coxph)
...
fit1 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+logentr, 
       data=immdef, censor_time=censyrs, test=coxph)
fit2 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+log(entry+1), 
...
test_that("survfit",{
          fit0 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+entry, 
                         data=immdef, censor_time=censyrs, test=coxph)
          fit1 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX), 
                         data=immdef, censor_time=censyrs, test=survdiff)
          fit2 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+entry, 
...
test_that("residual",{
  ...
  fit0 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+entry, 
                 data=immdef, censor_time=censyrs, test=coxph)
  fit1 <- rpsftm(Surv(progyrs,prog)~rand(imm,propX)+entry, 
                 data=immdef, censor_time=censyrs, test=survreg)
