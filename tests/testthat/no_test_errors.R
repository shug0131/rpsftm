...
immdef$propX <- with(immdef, I(1-xoyrs/progyrs))

test_that("Errors in test argument",{
expect_error( rpsftm(Surv(progyrs, prog)~rand(  propX~imm ),test=mantelhaen.test,immdef, censor_time = censyrs, 
...
test_that("No Surv object as a response",
          {
            expect_error( 
              rpsftm(progyrs~rand(  propX~imm ),
                     immdef, censor_time = censyrs),
...
test_that("Wrong type of Survival object",
          {
            expect_error( 
              rpsftm(Surv(progyrs, prog, type="left")~rand(  propX~imm ),
                     immdef, censor_time = censyrs),
...
test_that("Errors in arm >1",{
  expect_error( rpsftm(Surv(progyrs, prog)~rand(  propX+1~imm ),
                       immdef, censor_time = censyrs, 
...
test_that("Errors in randomisation out of {0,1}",{
  expect_error( rpsftm(Surv(progyrs, prog)~rand(  propX-1~imm+0.5 ),
                       immdef, censor_time = censyrs, 
                       low_psi=-1, hi_psi=1), "Invalid values for rx")
})
...
test_that("Censoring before Time warning",{
  expect_warning( rpsftm(Surv(progyrs, prog )~rand(  propX~imm ),
                         immdef, censor_time=censyrs*0.5,
                       low_psi=-1, hi_psi=1), "You have observed events AFTER censoring")
})

test_that("Variables on left and right terms",
         {
          expect_error( rpsftm(Surv(progyrs, prog)~rand(  I(1-xoyrs/progyrs)~imm )+progyrs,
                               immdef, censor_time = censyrs, 
...
test_that("No rand() terms",
          {
            expect_error( rpsftm(Surv(progyrs, prog)~imm,
                                 immdef, censor_time = censyrs, 
...
test_that("Too many rand() terms",
          {
            expect_error( rpsftm(Surv(progyrs, prog)~def*rand(  I(1-xoyrs/progyrs)~imm ),
                                 immdef, censor_time = censyrs, 
                                 low_psi=-1, hi_psi=1), "Exactly one rand\\(\\) term allowed")
...
test_that("rand() interaction",
          {
            expect_error( rpsftm(Surv(progyrs, prog)~def/rand( I(1-xoyrs/progyrs)~imm ),immdef, censor_time = censyrs, 
...
test_that("using covariates with special clashing names",
          {
            .arm <- rep(0:1,500)
            expect_warning(
              rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~ .arm )immdef, censor_time = censyrs, 
                     low_psi=-1, hi_psi=1 ),
              "Please rename"
            )
          }
...
test_that("More than 2 arms",
          {
            ...
            expect_error( rpsftm(Surv(progyrs, prog)~rand(  I(1-xoyrs/progyrs)~myarm ),immdef, censor_time = censyrs, 
...
test_that("less than 2 arms",
          {
            ...
            expect_error( rpsftm(Surv(progyrs, prog)~rand(  I(1-xoyrs/progyrs)~1 ),immdef, censor_time = censyrs, 
                                 low_psi=-1, hi_psi=1), "arm must have exactly 2 observed values")
          }
)
...
test_that("na actions",
          ...
          fit <- rpsftm(Surv(progyrs, prog)~rand(  I(1-xoyrs/progyrs)~imm ),immdef, censor_time = censyrs, na.action=na.fail,
                        low_psi=-1, hi_psi=1)
          expect_is(fit$psi, class="numeric")
          
          
          fit <- rpsftm(Surv(progyrs, prog)~rand(  I(1-xoyrs/progyrs)~myarm ),immdef, censor_time = censyrs, na.action=na.omit,
                 low_psi=-1, hi_psi=1)
          expect_is(fit$psi, class="numeric")
          expect_is(fit$na.action, class="omit")
          
          expect_error(rpsftm(Surv(progyrs, prog)~rand(  I(1-xoyrs/progyrs)~myarm ),immdef, censor_time = censyrs, na.action=na.fail,
                        low_psi=-1, hi_psi=1),
...
test_that("warning for poor initial starting values",{
    expect_warning( fit <- rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm ),immdef, censor_time = censyrs,
...
test_that("warning for non-convergencevalues",{
  expect_error( fit <- rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm )+entry,
                              immdef, censor_time = censyrs)
                  )
...
test_that("eval_z incorrect input - too small a number",
          {
            expect_error( rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm ),
...
imm2 <- immdef$imm
imm2[1:100] <- NA
rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm2 ),
       data=immdef, censor_time = censyrs)
imm3 <- factor(imm2, levels=0:1, labels=c("Cx","Rx"))
rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm3 ),
       data=immdef, censor_time = censyrs)

expect_error(rpsftm(Surv(progyrs, prog)~rand( imm~I(1-xoyrs/progyrs) ),
       data=immdef, censor_time = censyrs))
imm_3arms <- factor(rep(1:3, ceiling(1000/3))[1:1000])
expect_error(rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm_3arms ),
       data=immdef, censor_time = censyrs))
}
)
...
test_that("Checking of Treatment modifier",
          {
            ...
            expect_warning(rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm ),
                                  data=immdef,treat_modifier = mod, censor_time = censyrs),
                           "treat_modifier values are not all strictly positive"
                           )
            mod <- c(0,rep(2,nrow(immdef)-1))
            expect_warning(fit <- rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm ),
                                  data=immdef,treat_modifier =mod, censor_time = censyrs),
                           "treat_modifier values are not all strictly positive"
            )
...
test_that("problems with limits",
          {
expect_warning(rpsftm(Surv(progyrs, prog)~rand( I(1-xoyrs/progyrs)~imm ),immdef, censyrs,
